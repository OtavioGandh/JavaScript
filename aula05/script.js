// 1. Construir uma página para uma empresa de serviços. O operador irá digitar o salário de um cliente. Se o salário for maior ou igual que R$ 3.000,00, o programa irá escrever na tela “Oferecer Plano Alfa” e, caso seja menor, o programa escreverá “Oferecer Plano Beta”.

let salario = parse.Float(prompt("Digite o valor do salário"))
if (salario>= 3000){
...
let salarioCliente = parseFloat(prompt("Digite o salário do cliente: R$"))
...
let colesterolUsuario = parseFloat(prompt("Informe a taxa: "));
let taxaColesterol = 180;

if(colesterolUsuario <= taxaColesterol) {
    document.write("===== S A U D E  O K =====");
} else if(colesterolUsuario > taxaColesterol) {
    document.write(" V A M O S  P R O C U R A R  U M  M É D I C O ?")
} else {
    document.write(" @@@ D A D O  I N V Á L I D O @@@")
}
...
let valorMes = parseFloat(prompt("Digite o valor vendido na loja durante o mês: "))
    
    if (valorMes>=10.000){
        document.write(valorMes * 0.04)
    }
    else if (valorMes<10.000){
        document.write(valorMes * 0.02) 
    }
    else{ 
        Document.write("Digite um valor válido!!!")
    }
